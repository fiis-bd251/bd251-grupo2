...
DROP TABLE IF exists Propuestas_de_compra CASCADE;
DROP TABLE IF exists Seguimientos_de_compra CASCADE;
DROP TABLE IF exists Ordenes_de_compra CASCADE;
DROP TABLE IF exists Compras CASCADE;
DROP TABLE IF exists Reclamos CASCADE;
DROP TABLE IF exists Cantidades_acumuladas CASCADE;
...
END $$;
CREATE TYPE estado_enum AS ENUM ('C', 'P', 'I', 'R');

DO $$ BEGIN
    DROP TYPE IF EXISTS tipo_boquilla_enum;
END $$;
CREATE TYPE tipo_boquilla_enum AS ENUM ('MLD01', 'MLD02', 'MLD03', 'MLD04', 'MLD05');

DO $$ BEGIN
    DROP TYPE IF EXISTS tipo_envase_enum;
END $$;
...
END $$;
CREATE TYPE categoria AS ENUM ('A', 'B', 'C');

DO $$ BEGIN
    DROP TYPE IF EXISTS estado_propuesta_compra;
END $$;
CREATE TYPE estado_propuesta_compra AS ENUM ('PND', 'APB', 'RCZ');

DO $$ BEGIN
    DROP TYPE IF EXISTS estado_orden_compra;
END $$;
CREATE TYPE estado_orden_compra AS ENUM ('SNV', 'PND', 'PGD', 'CNL');

DO $$ BEGIN
    DROP TYPE IF EXISTS estado_compra;
END $$;
CREATE TYPE estado_compra AS ENUM ('SRV', 'PRC', 'ACP', 'RCZ');

DO $$ BEGIN
    DROP TYPE IF EXISTS estado_seguimiento_compra;
END $$;
CREATE TYPE estado_seguimiento_compra AS ENUM ('PND', 'TMP', 'RTS', 'CNL');

DO $$ BEGIN
    DROP TYPE IF EXISTS estado_reclamo;
END $$;
CREATE TYPE estado_reclamo AS ENUM ('ATN', 'PND', 'TRM');

DO $$ BEGIN
    DROP TYPE IF EXISTS objetivo_reclamo;
END $$;
CREATE TYPE objetivo_reclamo AS ENUM ('RPS', 'DVL');
...
create table Solicitudes_de_abastecimiento (
...
CREATE TABLE Dosificado (
    ...
    ESTADO estado_enum,
    ID_LOTE_PRODUCTO INT UNIQUE NOT NULL REFERENCES Lote_Producto(ID_LOTE_PRODUCTO),
    ID_EMPLEADO INT UNIQUE NOT NULL REFERENCES Personas(ID_PERSONA)
);

CREATE TABLE DosificadoxLote_insumo (
    ID_DOSIFICADO INT NOT NULL REFERENCES Dosificado(ID_DOSIFICADO),
    ID_LOTE_INSUMO INT NOT NULL REFERENCES Lote_Insumo(ID_LOTE_INSUMO),
...
CREATE TABLE Proceso_Recurrente (
    ...
    HORA_INICIO TIME NOT NULL,
    TIEMPO_PROCESO NUMERIC(3) CHECK (TIEMPO_PROCESO > 0),
    PESO_INICIAL NUMERIC(4,1) CHECK (PESO_INICIAL > 0),
    MERMA NUMERIC(4,1) CHECK (MERMA >= 0),
    ESTADO estado_enum,
    ID_LOTE_PRODUCTO INT UNIQUE NOT NULL REFERENCES Lote_Producto(ID_LOTE_PRODUCTO)
);
...
CREATE TABLE Mezclado (
    ...
    PORCENTAJE_HUMEDAD NUMERIC(1,2) CHECK (PORCENTAJE_HUMEDAD > 0),
    CANTIDAD_AGUA NUMERIC(3) CHECK (CANTIDAD_AGUA > 0),
    ID_EMPLEADO INT UNIQUE NOT NULL REFERENCES Personas(ID_PERSONA),
    ID_EJEMPLAR_MAQUINA INT UNIQUE NOT NULL REFERENCES Ejemplar_Maquina(ID_EJEMPLAR_MAQUINA),
    ID_FORMULACION INT UNIQUE NOT NULL
);
...
CREATE TABLE Moldeado (
    ...
    PRESION NUMERIC(3,2) CHECK (PRESION > 0),
    VELOCIDAD_CORTE NUMERIC(3,2) CHECK (VELOCIDAD_CORTE > 0),
    TAMANO_CORTE NUMERIC(3,2) CHECK (TAMANO_CORTE > 0),
    ID_EMPLEADO INT UNIQUE NOT NULL REFERENCES Personas(ID_PERSONA),
    ID_EJEMPLAR_MAQUINA INT UNIQUE NOT NULL REFERENCES Ejemplar_Maquina(ID_EJEMPLAR_MAQUINA)
);
...
CREATE TABLE Secado (
    ...
    TEMPERATURA_INICIAL NUMERIC(3,3) CHECK (TEMPERATURA_INICIAL > 0),
    TEMPERATURA_FINAL NUMERIC(3,3) CHECK (TEMPERATURA_FINAL > 0),
    PORCENTAJE_HUMEDAD NUMERIC(1,2) CHECK (PORCENTAJE_HUMEDAD > 0),
    ID_EMPLEADO INT UNIQUE NOT NULL REFERENCES Personas(ID_PERSONA),
    ID_EJEMPLAR_MAQUINA INT UNIQUE NOT NULL REFERENCES Ejemplar_Maquina(ID_EJEMPLAR_MAQUINA)
);
...
create table Propuestas_de_compra (
	ID_PROPUESTA_COMPRA SERIAL primary key,
	ID_EMPLEADO INT not null,
	ID_PROVEEDOR INT not null,
	ID_SOLICITUD_ABASTECIMIENTO INT not null,
	FECHA_ACUERDO_ENTREGA DATE not null,
	HORA_ACUERDO_ENTREGA TIME not null,
	FECHA_PROPUESTA_COMPRA DATE not null,
	HORA_PROPUESTA_COMPRA TIME not null,
	ESTADO estado_propuesta_compra not null,
	DESCUENTO NUMERIC(4,2) CHECK (DESCUENTO >= 0),
	MONTO_TOTAL NUMERIC(7,2) NOT NULL CHECK (MONTO_TOTAL >= 0),
	
	foreign key (ID_EMPLEADO) references Empleados(ID_EMPLEADO),
	foreign key (ID_PROVEEDOR) references Proveedores(ID_PROVEEDOR),
	foreign key (ID_SOLICITUD_ABASTECIMIENTO) references Solicitudes_de_abastecimiento(ID_SOLICITUD_ABASTECIMIENTO)
);
...
create table Ordenes_de_compra (
	...
	foreign key (ID_EMPLEADO) references Empleados(ID_EMPLEADO),
	foreign key (ID_PROPUESTA_COMPRA) references Propuestas_de_compra(ID_PROPUESTA_COMPRA)
);
...
create table Compras (
	...
	foreign key (ID_LOTE_INSUMO) references Lote_insumo(ID_LOTE_INSUMO)
);

create table Seguimientos_de_compra (
	ID_SEGUIMIENTO_COMPRA SERIAL primary key,
	ID_COMPRA INT not null,
	FECHA_INGRESO DATE not null,
	HORA_INGRESO TIME not null,
	ESTADO estado_seguimiento_compra not null,
...
create table Reclamos (
	...
	ID_LOTE_INSUMO INT not null,
	ID_EMPLEADO INT not null,
	MONTO_DEVUELTO numeric(6,2) CHECK (MONTO_DEVUELTO >= 0),
	FECHA_ENTREGA DATE not null,
	HORA_ENTREGA TIME not null,
	ESTADO_RECLAMO estado_reclamo not null,
	OBJETIVO_RECLAMO objetivo_reclamo not null,
	
	foreign key (ID_EMPLEADO) references Empleados(ID_EMPLEADO),
	foreign key (ID_INSPECCION) references Inspeccion_Lote_Insumo(ID_INSPECCION)
);

create table InsumosxProveedores (
    PRECIO_REFERENCIAL NUMERIC(4,2) CHECK (PRECIO_REFERENCIAL >= 0) NOT NULL,
    ID_PROVEEDOR INT NOT NULL,
